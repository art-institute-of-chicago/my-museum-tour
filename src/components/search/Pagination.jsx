...
function Pagination() {
  ...
  const pages = Array.from(Array(pagination?.total_pages ?? 1), (_, index) => index + 1);
  console.log(pages);
  ...
  return (
    <nav className="m-paginator">
      ...
      <ul className="m-paginator__pages">
        {pages.map((page) => <PageNumber key={page} page={page} is_current_page={page === pagination?.current_page} />)}
      </ul>
